...
export function TableApplicationsElement({ vacancyId }) {
  ...
  if (applications.length === 0) {
    return (
      <div className="mt-15 flex flex-col items-center text-center gap-3">
        <h1 className="font-konkhmer px-6 py-2 bg-black text-white">
          No responses to the vacancy yet(
        </h1>
        <a
          href="/my-vacancies"
          className="bg-blue font-konkhmer text-white rounded-xl px-6 py-2"
        >
          Back
        </a>
      </div>
  ...
  return (
    <div className="relative overflow-x-auto mt-13">
      <table className="w-full t-label text-center">
        <thead className="t-label text-black  bg-green">
          <tr>
            ...
            <th
              scope="col"
              className="px-6 py-3 border-r border-black/30 bg-blue text-white"
            >
              Status
            </th>
          </tr>
        ...
        <tbody className="border-l border-r border-b border-black/30">
          {applications.map((applicant) => (
            <tr key={applicant.applicationId} className="bg-white font-kosugi">
              <td
                scope="row"
                className="px-6 py-4 border-r border-black/30 text-black "
              >
              ...
              <td className="px-6 py-4 border-r border-black/30">
                <p className="overflow-hidden">{applicant.applicantEmail}</p>
              </td>
              <td className="px-6 py-4 border-r border-black/30">
                <a
              ...
              <td className="px-6 py-4 border-r border-black/30">
                ...
                >
                  {applicant.applicationStatus === "PENDING" ? (
                    <select
                      value={
                        selectedStatuses[applicant.applicationId] || "ACCEPT"
                      }
                      onChange={(e) =>
                        setSelectedStatuses({
                          ...selectedStatuses,
                  ...
                  {applicant.applicationStatus !== "PENDING" ? null : (
                    <button
                      type="button"
