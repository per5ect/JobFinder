...
const BACKEND_USER_API_URL = `${API_BASE_URL2}/user`;
...
export const getAllMatchesVacancies = async (
  filters = {},
  page = 0,
  size = 10,
) => {
  ...
  const response = await apiUser.get(
    `/getMatchingVacancies?${params.toString()}`,
  );
  return response.data;
};
...
export const updateUserPersonalInformation = async (updatedData) => {
  const response = await apiUser.patch(
    "/my-account/update-personal-info",
    updatedData,
  );
  return response.data;
};
