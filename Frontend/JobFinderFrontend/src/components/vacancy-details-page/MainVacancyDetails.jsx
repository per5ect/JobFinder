...
export function MainVacancyDetails() {
  ...
  return (
    <main className="mt-22 mb-22">
      <Layout>
        <section className="flex flex-col xl:flex-row xl:items-center w-full xl:justify-between gap-8">
          <div className="flex flex-col items-start xl:max-w-[554px] gap-5">
            <h3 className="t-display text-black">{vacancyData.title}</h3>
            <div className="flex flex-row items-center gap-4">
              <p className="t-heading text-black">Company:</p>
        ...
        <div className="mt-18 flex flex-row justify-center w-full">
          <h1 className=" bg-black text-white t-display px-7">
            Job description
          </h1>
        </div>
        ...
        <section className="mt-18 w-full">
          <h3 className="t-display text-start">What do we expect from you?</h3>
          <VacancyRequirements vacancyData={vacancyData} />
        </section>
        <section className="mt-18 w-full">
          <h3 className="t-display text-start">What's in store for you? </h3>
          <VacancyOfferings vacancyData={vacancyData} />
        </section>
        <section className="mt-18 w-full flex flex-col items-center gap-10">
          <h1 className="bg-black text-white t-display px-7">Contacts</h1>
          <VacancyCompanyContacts vacancyData={vacancyData} />
          {role === "ROLE_USER" ? (
            <button
              className="w-full xl:w-1/2 mt-12 py-8 bg-green button-kosugi rounded-2xl cursor-pointer t-heading transition-colors ease-in-out hover:bg-green/90"
            ...
            <a
              href="/user-login"
              className="w-full xl:w-1/2 mt-12 py-8 bg-green button-kosugi rounded-2xl text-center t-heading"
            >
              Sign In to apply
            </a>
          ) : null}
