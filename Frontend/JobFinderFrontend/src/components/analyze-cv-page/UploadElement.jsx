...
export function UploadElement({ setAnalyseStep, analyseResult }) {
  ...
  return (
    <div className="text-center w-full max-w-[750px]">
      <h2 className="t-heading bg-green py-9 rounded-t-xl">
        {isUploaded
          ? "Analyze your resume"
      ...
      <div className="w-full flex flex-col gap-10 px-8 py-5 border border-black/20 border-t-0 rounded-b-xl min-h-[400px]">
        <div className="w-full flex flex-col items-start mt-4">
          ...
          <p className="t-body text-black ">
            {isUploaded
              ? "🤖 Artificial intelligence analyzes your experience,skills and education"
              : " 🤖 Select your resume in a pdf format"}{" "}
          </p>
        </div>
        <div
          {...getRootProps()}
        ...
        >
          ...
          {isDragActive ? (
            ...
            <button
              className="text-blue font-kosugi border border-blue px-4 py-3 rounded-xl "
              onClick={handleRemoveFile}
            >
              {selectedFile.name}
            </button>
          ) : (
            <div className="flex flex-col items-center t-label gap-3">
              <img src="/icons/upload.svg" alt="" />
              <p className="text-black">Drag your resume to start uploading</p>
              <p>------ OR ------</p>
              <span className="text-blue border border-blue px-3 py-2 rounded-xl">
                Browse files
              </span>
            </div>
          )}
        ...
        {loading && (
          <div className="flex flex-col items-center font-kosugi gap-4">
            <Loader />
            <p>
              🔍 It takes a few seconds and then we'll find the best jobs for
              you.
            </p>
          </div>
        )}
