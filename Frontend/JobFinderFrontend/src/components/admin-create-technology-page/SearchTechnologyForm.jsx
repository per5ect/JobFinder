...
export function SearchTechnologyForm() {
  ...
  const handleDeleteTechnology = async (technologyId) => {
    try {
      ...
      setTechnologyList((prevList) =>
        prevList.filter((t) => t.id !== technologyId),
      );
    }
  };
  ...
  return (
    <div className="pt-12.5 border-t-1 border-[#222A24]/10">
      <form action="" className="flex flex-col gap-4">
        <label htmlFor="search-technology" className="t-body text-black">
        ...
        {filteredTechnologies.length > 0 ? (
          <div className="flex flex-wrap gap-2.5">
            {filteredTechnologies.map((technology) => (
              <span
                key={technology.id}
                className="bg-blue font-konkhmer text-white border rounded-2xl px-7.5 py-2 cursor-pointer"
          ...
          <p className="text-white bg-black  font-konkhmer px-5 py-3 text-center rounded-2xl">
