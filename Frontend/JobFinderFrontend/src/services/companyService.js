...
export const getVacancyApplications = async (vacancyId) => {
  const response = await apiCompany.get(
    `/my-vacancies/${vacancyId}/applications`,
  );
  return response.data;
};

export const acceptApplicant = async (applicantId) => {
  const response = await apiCompany.patch(
    `/applications/${applicantId}/accept`,
  );
  return response.data;
};
...
export const rejectApplicant = async (applicantId) => {
  const response = await apiCompany.patch(
    `/applications/${applicantId}/reject`,
  );
  return response.data;
};
